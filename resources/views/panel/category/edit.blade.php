...
<div class="row">
    ...
    <div class="col-xl-12 col-md-12 mb-4 p-md-5 bg-white">
        ...
        <form action="{{ route('category.update', ['category'=>$category]) }}" method="POST">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control" id="parent_id" name="parent_id">
                        <option value="" {{ $category->parent_id == NULL ? "selected" : "" }}>بدون والد</option>
                ...
                <div class="form-group col-md-3">
                    ...
                    <select id="attributeIsFilterSelect" name="attribute_is_filter_ids[]" class="form-control" data-title="انتخاب ویژگی های فیلتر" multiple data-live-search="true">
                        ...
                        @if($categoryAttribute->pivot->is_filter)
                        <option value="{{ $categoryAttribute->id }}">{{ $categoryAttribute->name }}</option>
                        @endif
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group col-md-3">
                    ...
                    <select id="variationSelect" name="variation_id" class="form-control" data-title="انتخاب ویژگی های متغیر" data-live-search="true">
                        @foreach ($category->attributes as $categoryAttribute)
                        @if($categoryAttribute->pivot->is_variation)
                        <option value="{{ $categoryAttribute->id }}">{{ $categoryAttribute->name }}</option>
                        @endif
                        @endforeach
                    </select>
            ...
            <button class="btn btn-outline-primary mt-5" type="submit">بروز رسانی</button>
